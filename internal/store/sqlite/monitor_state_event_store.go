...
func (s *Store) CountMonitorStateEvents(ctx context.Context) (int64, error) {
	var count int64
	err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM monitor_state_events`).Scan(&count)
	if err != nil {
		if isMalformedSQLiteError(err) {
			return 0, nil
		}
		return 0, fmt.Errorf("count monitor state events: %w", err)
	}
	return count, nil
}

func (s *Store) ListMonitorStateEventsPaginated(ctx context.Context, limit, offset int) ([]MonitorStateEvent, error) {
	if limit <= 0 {
		limit = 50
	}
	if offset < 0 {
...
`, limit, offset)
	if err != nil {
		...
		return nil, fmt.Errorf("list monitor state events paginated: %w", err)
	}
	defer rows.Close()
